...
export class DoorwayPlacementEngine {
  ...
  static realignDeclaredHallwayOpenings(
    openings = [],
    sectors = [],
    customConfig = {}
  ) {
    ...
    return openings.map(opening => {
      ...
      const sector = byId.get(opening.hostId);
      const declared = this._declaredHallwayOpening(sector, config);
      if (!declared) return opening;

      return {
        ...opening,
        hallwayHostId: declared.hostId,
        declaredHallwayFace: declared.face,
        exactFaceRequired: true,
        face: declared.face,
        orientation: this._doorOrientationForFace(declared.face),
        location: { ...declared.location },
        requestedLocation: { ...declared.location },
        realignedAfterLayout: true
      };
    });
  }
